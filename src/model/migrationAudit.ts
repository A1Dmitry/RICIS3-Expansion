...
function repairNodeTitle(node: ProblemNode): { title: string; changed: boolean } {
  ...
  if (catalogMatch && catalogMatch.title) {
    if (title !== catalogMatch.title) {
      return { title: catalogMatch.title, changed: true };
    }
  }

  // 2. Identify placeholders, stubs, IDs as titles, or unformatted text
  const isStub =
    !title ||
    title.length < 3 ||
  ...
  if (isStub) {
    // Generate a meaningful title based on zone and target function / hint
...
function pickDefaultRootForNode(node: ProblemNode, nodeMap: Map<string, ProblemNode>): string {
  const zone = node.zoneIds?.[0] || 'math';
  
  // Try to find an existing connected node in the same zone first
  for (const candidate of nodeMap.values()) {
    if (candidate.id === node.id) continue;
    if (candidate.zoneIds?.includes(zone) && canReachRoot(candidate.id, nodeMap)) {
      return candidate.id;
    }
  }

  // Fallback to core roots
  if (['informatics', 'medicine', 'pharmacology', 'economics', 'ethics'].includes(zone)) {
...
export function auditAndFixMapGraph(map: MapState): { map: MapState; report: MigrationAuditReport } {
  ...
  map.nodes.forEach(orig => {
    ...
    // Zone responsibility alignment
    const fullText = (node.title + ' ' + (node.targetFunction || '') + ' ' + (node.description || '')).toLowerCase();
    if (/agi|рицис|ricis|целевая функция agi|искусственный интеллект/i.test(fullText)) {
      if (!node.zoneIds.includes('informatics')) {
        node.zoneIds = ['informatics', ...node.zoneIds.filter(z => z !== 'informatics')];
      }
    }
    if (/якобиан|jacobian|топологическ|инвариант|деление на ноль|гипотеза якобиана/i.test(fullText)) {
      if (!node.zoneIds.includes('math')) {
        if (!node.zoneIds.includes('informatics')) {
          node.zoneIds = ['math', ...node.zoneIds];
        } else {
          node.zoneIds = ['informatics', 'math'];
        }
      }
    }
  ...
  // 2. Audit Connectivity & Reconnect Orphans
  for (const node of nodeMap.values()) {
    ...
    if (!CORE_ROOT_IDS.has(node.id) && !canReachRoot(node.id, nodeMap)) {
      const rootParentId = pickDefaultRootForNode(node, nodeMap);
  ...
  for (const node of nodeMap.values()) {
    // Filter parent's dependentIds to ensure children exist and list parent in their dependencyIds
    const validDependents = Array.from(new Set(
      node.dependentIds.filter(childId => {
        const childNode = nodeMap.get(childId);
        if (!childNode) return false;
        if (!childNode.dependencyIds.includes(node.id)) {
          childNode.dependencyIds.push(node.id);
        }
        return true;
      })
    ));
    node.dependentIds = validDependents;
  }
